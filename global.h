...
inline static void quantizeAVX2(char* data_ptr, uint32_t size) {
    // check alignment
    char data_clone_ptr [size] alignas(256);
    *data_clone_ptr = *data_ptr;
    ...
    __m256* input_avx = (__m256*) data_clone_ptr;
    ...
    uint32_t remain_cnt = size;

    for (uint32_t i = 0; i <= size / sizeof(__m256); i++) {
        ...
        if (remain_cnt >= 8) {
            ...
            for (uint32_t j = 0; j < remain_cnt; j++) {
                int_data_ptr[i * 8 + j] = output[j];
            }
            break;
        }
    }
...
inline static void dequantizeAVX2(char* data_ptr, uint32_t size) {
    ...
    *data_clone_ptr = *data_ptr;
    ...
    __m256i* input_avx = (__m256i*) data_clone_ptr;
    __m256 factor_in_avx = _mm256_broadcast_ss(factor_ptr);

    uint32_t remain_cnt = size;

    for (uint32_t i = 0; i < size / sizeof(__m256i); i++) {
        __m256 scaled_output_avx = _mm256_cvtepi32_ps(input_avx[i]);
